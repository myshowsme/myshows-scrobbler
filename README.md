...
- Старые библиотеки Plex, собранные на неподдерживаемых агентах (`com.plexapp.agents.*`), включая аниме-сканер HAMA, тоже поддерживаются.
...
Скробблеру не обязательно жить на вашем компьютере. Если дома есть NAS или домашний сервер, поставьте его туда — и отметки будут уходить в MyShows, даже когда компьютер выключен. Это то, что нужно, если вы смотрите с телевизора или телефона.

Единственное ограничение: изнутри контейнера не видно программ, запущенных на вашем компьютере, поэтому «Локальный плеер», VLC и mpv так работать не будут. Plex, Jellyfin, Emby и Kodi — будут: к ним скробблер подключается по сети.

### Docker

Собирать ничего не нужно, готовый образ уже опубликован. Он подходит и обычным процессорам Intel/AMD, и процессорам ARM — на них работает большинство NAS, а также Raspberry Pi.

**1. Создайте папку для настроек.** В ней скробблер будет хранить свой конфиг, чтобы тот не пропадал при обновлении:
...
Путь замените на свой — на NAS папки обычно лежат в `/volume1`, `/Volume1` или `/share`, посмотреть можно командой `ls /`.

Вторая команда отдаёт папку тому пользователю, под которым работает контейнер. Без неё скробблер не сможет сохранить настройки и будет забывать токен при каждом перезапуске. На Windows и macOS этот шаг не нужен.
...
`--restart unless-stopped` означает, что скробблер сам поднимется после перезагрузки NAS.

**3. Откройте веб-интерфейс** по адресу сервера и порту 3000 — например, `http://192.168.1.50:3000`. Дальше всё как в десктопном приложении: вставьте [токен MyShows](https://myshows.me/profile/watch-history/) и включите нужный источник.

Одна тонкость при подключении медиасервера, который стоит на той же машине: указывайте его адрес в сети (`http://192.168.1.50:8096`), а не `localhost`. Для контейнера `localhost` — это он сам, а не ваш Jellyfin.

Проверить, что всё запустилось, можно командой `docker logs myshows-scrobbler` — там должна быть строка `Server listening`.

#### Через docker compose

В репозитории лежит готовый [docker-compose.yml](docker-compose.yml):

```bash
docker compose up -d
```

Права на папку `data` нужны те же, что и выше.

#### Собрать образ самому

Нужно, только если правите код: раскомментируйте `build: .` в [docker-compose.yml](docker-compose.yml), и `docker compose up -d` соберёт образ из локальных исходников.
