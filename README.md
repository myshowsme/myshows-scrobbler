...
Засчитываются только сеансы, у которых имя пользователя или его ID совпадает с записью из списка: для Plex это `User.title` и `User.id`, для Emby и Jellyfin — `UserName` и `UserId`. Сравнение без учёта регистра и пробелов по краям. Пустой (или состоящий только из пробелов) `user_filter` — засчитываются все зрители. Сеанс, в котором сервер не сообщил пользователя, при заданном фильтре не засчитывается.
